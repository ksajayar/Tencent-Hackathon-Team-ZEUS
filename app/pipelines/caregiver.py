...
# §17 set medication: a fixed menu, not free-text schedule parsing (see the
# comment on CAREGIVER_MEDICATION_ASK_SCHEDULE in i18n/strings.py for why).
# Verified against the real next_occurrence()/rrulestr() machinery this
# session - comma-separated BYHOUR cycles correctly for the twice/three-times
# options, no changes needed to app/services/rrule.py.
_SCHEDULE_MENU: dict[str, tuple[str, dict[str, str]]] = {
    "1": (
        "FREQ=DAILY;BYHOUR=8;BYMINUTE=0",
        {"en": "once a day, morning", "zh-Hans": "每天一次，早上"},
    ),
    "2": (
        "FREQ=DAILY;BYHOUR=20;BYMINUTE=0",
        {"en": "once a day, evening", "zh-Hans": "每天一次，晚上"},
    ),
    "3": (
        "FREQ=DAILY;BYHOUR=8,20;BYMINUTE=0",
        {"en": "twice a day, morning and evening", "zh-Hans": "每天两次，早晚各一次"},
    ),
    "4": (
        "FREQ=DAILY;BYHOUR=8,13,20;BYMINUTE=0",
        {"en": "three times a day, with meals", "zh-Hans": "每天三次，三餐时"},
    ),
}
...
async def _continue_medication(
    ...
    if step == "schedule":
        choice = text.strip()
        if choice not in _SCHEDULE_MENU:
        ...
        rrule, label = _SCHEDULE_MENU[choice]
        data = {**data, "schedule_rrule": rrule, "schedule_label": _pick(label, reply_language)}
...
def _render_instruction(data: dict) -> tuple[str, str]:
    """Plain-language instruction from the fixed schedule menu choice - not
    generated by an LLM (SAFETY-1: medication instructions must be exact,
    not paraphrased by a model)."""
    label_en = _pick(_SCHEDULE_MENU[_menu_key_for(data["schedule_rrule"])][1], "en")
    label_zh = _pick(_SCHEDULE_MENU[_menu_key_for(data["schedule_rrule"])][1], "zh-Hans")
    return (
        f"Take {data['dose_text']}, {label_en}.",
        f"{label_zh}服用{data['dose_text']}。",
    )


def _menu_key_for(rrule: str) -> str:
    for key, (candidate_rrule, _label) in _SCHEDULE_MENU.items():
        if candidate_rrule == rrule:
            return key
    raise ValueError(f"unknown schedule rrule: {rrule}")
